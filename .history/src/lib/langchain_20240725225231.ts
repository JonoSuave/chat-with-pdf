...
export async function generateEmbddingsInPineconeVectorStore(docId: string) {
	...
	const generateLangchainCompletion = async (docId: string, question: string) => {
		...
		const promptTemplate = new ChatPromptTemplate.fromMessages([...chatHistory, // Insert the actial chat history here
		["user", "{input}"],
		[
			"user",
			"Given the above conversation, generate a search query to look up in order to get information rel"
		]
	]);
	};
}
