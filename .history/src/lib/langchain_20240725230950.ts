...
export async function generateEmbddingsInPineconeVectorStore(docId: string) {
	...
	const generateLangchainCompletion = async (docId: string, question: string) => {
		...
		// Create the main retrieval chain that combines the history-aware retriever and document combineing chains
		console.log("--- Creating the main retrieval chain ---");
		const conversationalRetrievalChain = await createRetrievalChain({
			retrieverChain: historyAwareRetrieverChain,
			documentChain: historyAwareCombineDocsChain,
		});
	};
}
